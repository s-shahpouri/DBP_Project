...
class Dual3DCNN7(nn.Module):  # Removed one block to reduce depth
    def __init__(self, width=128, height=128, depth=128):
        super(Dual3DCNN6, self).__init__()
        
        self.initializer = nn.init.kaiming_normal_
        num_filters = [16, 32, 64]  # Removed the 128 filters block
        num_blocks = len(num_filters)
        ...
        # Adjust the input features for the fully connected layer accordingly
        self.fc = nn.Sequential(
            nn.Linear(in_features=num_filters[-1]*2*2*2, out_features=128),  # Adjusted for global pooling output
            nn.Dropout(p=0.5),
            nn.ReLU(inplace=True),
            nn.Linear(in_features=128, out_features=64),
            nn.Dropout(p=0.5),
        ...
        self.global_pool = nn.AdaptiveAvgPool3d((2, 2, 2))  # Adjusted global pooling size
